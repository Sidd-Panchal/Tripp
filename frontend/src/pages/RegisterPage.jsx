...
const RegisterPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      showSuccess('Welcome to Trrip AI! Let\'s build some amazing travel plans.');
      navigate('/dashboard');
    } catch (error) {
      showError(error.message || 'Registration failed. Email might already exist.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        <div className="flex flex-col items-center gap-2 mb-6">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">
            Start automating your itineraries free
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            {errors.name && (
              <p className="text-[10px] text-rose-500 font-semibold pl-1">{errors.name.message}</p>
            )}
          </div>
